...
func wall_jump_check(wall_axis):
	if Input.is_action_just_pressed("jump"):
		...
		state = move_state
		jump(jump_force * .75, false)
		var wall_jump_effect_postion = global_position + Vector2(wall_axis * 3.5, -2)
		var wall_jump_effect = Utils.instantiate_scene_on_level(WallJumpEffectScene, wall_jump_effect_postion)
...
func apply_friction(delta):
	...
	else: velocity.x = move_toward(velocity.x, 0, air_friction * delta)
...
func update_animations(input_axis):
	...
	#jump animation needs to be at the end of function to override other checks.
	#this code will not work for jumping animations with more than 1 frame.
	if not is_on_floor():
...
func _on_hurtbox_hurt(hitbox, damage):
	...
	#the invincible setter is tied to the blinkinganimationplayer instead of in this funciton.
